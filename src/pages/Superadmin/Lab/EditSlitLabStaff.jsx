...
export default function EditSlitLabStaff() {
  const { id } = useParams();
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchSlitLabStaffById(id));
    }
  }, [dispatch, id]);

  useEffect(() => {
    if (slitLabStaffMember && slitLabStaffMember._id === id) {
      setFormData({
        name: slitLabStaffMember.name || '',
        email: slitLabStaffMember.email?.replace(/^deleted-\d+-/, '') || '',
        phone: slitLabStaffMember.phone || '',
        username: slitLabStaffMember.username?.replace(/^deleted-\d+-/, '') || '',
        password: '',
        confirmPassword: ''
      });
    }
  }, [slitLabStaffMember, id]);
  ...
  }, [updateSlitLabStaffSuccess, dispatch, navigate]);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateSlitLabStaff({ id, payload }));
  };
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6 sm:mb-8">
          ...
          <p className="text-slate-600 text-sm">
            Update contact information or reset credentials for this SLIT therapy specialist.
          </p>
        </div>
