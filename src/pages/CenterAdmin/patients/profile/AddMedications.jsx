...
export default function AddMedications() {
  ...
  const { loading, addMedicationSuccess, patientDetails, error } = useSelector(
    (state) => state.centerAdmin
  );
  ...
  const handleSubmit = async (event) => {
    ...
    if (invalidMedication) {
      setSubmissionState({
        status: "error",
        message: "Please fill in the medicine name, dosage, and duration for each entry.",
      });
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        dispatch(resetCenterAdminState());
        navigate(`/dashboard/CenterAdmin/patients/profile/ViewProfile/${id}`);
      }, 1500);
    } catch (submitError) {
      const errorMessage =
